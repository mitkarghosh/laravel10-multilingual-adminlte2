	...
	@section('content')
		...
		<section class="common_banner">
			<div class="bannerbox">
				...
				<img src="{{asset('images/site/sample/Header-new.JPG')}}" alt="Banner">
				</figure>
			</div>
		</section>
		...
		<section class="section is-home-page-class">
			<div class="container">
						...
						@foreach ($item->products as $keyProduct => $valProduct)
									...
									@foreach($valProduct->productAttributes as $keyAttributes => $valAttributes)
										<li>
											<a href="javascript: void(0);" class="add_to_basket" id="anchor_prod_id_{{$valProduct->id}}" data-productid="{{Helper::customEncryptionDecryption($valProduct->id)}}" data-prodid="{{$valProduct->id}}" data-showingredients="" data-attributeid="{{Helper::customEncryptionDecryption($valAttributes->id)}}" data-hasattribute="Y" data-drinkid="" data-specialid="" data-proprice="{{$valProduct->price}}">
												...
												<div class="tt_fright"><span class="blue">CHF <span id="updated_prod_id_{{$valProduct->id}}" class="havingAttribute_{{$valProduct->id}}" data-bp="{{Helper::formatToTwoDecimalPlaces($valAttributes->price)}}">{{Helper::formatToTwoDecimalPlaces($valAttributes->price)}}</span> <i class="plus">+</i></span></div>
											</a>
										</li>
									@endforeach
									</ul>
							...
							@if ($valProduct->has_attribute == 'N')
								@if ($valProduct->show_ingredients == 'N')
                                    ...
                                    @php $addonCheckExistOrNot=0; $adid=1; @endphp
									@foreach ($valProduct->productMenuTitles as $menuTitle)
										@if($menuTitle->addon_id>0)
										   @php $addonCheckExistOrNot=1; @endphp
										@endif 
										@break
									@endforeach
									@if($addonCheckExistOrNot==1)
													@php $singleDropDownPrice = 0; $check_is_menu=0; @endphp
													<div class="meal_opt dropdown-custom-menu-{{$valProduct->id}}">
														
														@foreach ($valProduct->productMenuTitles as $menuTitle)

														@if(Helper::addoStatusCheck($menuTitle->addon_id))

															@php $newtitle=!empty($menuTitle->local[0])?$menuTitle->local[0]->local_title:'' @endphp	
															@if ($menuTitle->is_multiple == 'N' && $newtitle)	 
															<label class="labelWrap">
																...
																<select name="dropdown_menu_value" id="dropdown_menu_value" class="singleDropDown dropdown_menu_value_{{$valProduct->id}}" autocomplete="off" data-prodid="{{$valProduct->id}}">
																	...
																	@if($valMenuValue->sub_addon_status && Helper::addoStatusCheck($valMenuValue->sub_addon_id))
																	@php $check_is_menu=1; @endphp	
																	@php if ($keuMenuValue == 0) {$singleDropDownPrice += $valMenuValue->price;} @endphp
																		@php $newtitlevalue=!empty($valMenuValue->local[0])?$valMenuValue->local[0]->local_title:'' @endphp	
																		<option value="{{$valMenuValue->id.'|'.$menuTitle->id}}" data-ddprice="{{$valMenuValue->price}}">{{$newtitlevalue}} @if ($valMenuValue->price > 0)(+ {{Helper::formatToTwoDecimalPlaces($valMenuValue->price)}} CHF) @endif</option>
																	@endif
																		@endforeach
																</select>
															</label>
												...
												@php
													...
													if($check_is_menu==0){ @endphp
														<style>
															.dropdown-custom-menu-{{$valProduct->id}}{
																display:none;
															}
